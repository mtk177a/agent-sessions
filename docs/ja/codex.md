...
出力するイベントには、正規のロールアウト行の記録時刻を使う。\
一つの完了済みツール項目から生じる呼び出しと結果には同じ `recorded_at` を付け、別々の呼び出し時刻と結果時刻を確定したとは扱わない。\
入力の有無は、項目の種類に対応する `command`、`arguments`、`input`、または旧形式のローカルシェル操作に含まれる `command` から判定する。\
受け付ける旧形式の行で想定される入力フィールドが欠けている場合は、入力が存在しなかったと断定せず `input_state: unavailable` を報告する。
